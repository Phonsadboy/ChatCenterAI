...
- `get_instruction_inventory`
  ...
  - เป็น inventory/preview เท่านั้น ไม่ถือว่าอ่านข้อมูลครบทั้งชุด

- `search_instruction_content`
  - ค้น role/catalog/scenario/data items ด้วย keyword/semantic

- `get_instruction_data_snapshot`
  - อ่านข้อมูลจริงของ data items แบบเต็มหรือเป็น chunk พร้อม `complete`, `hasMore`, `nextStartRow`, `contentHash`
  - ต้องใช้ก่อนงานที่แก้หลายจุด, replace ทั้ง text, ลบ/ย้าย/แก้ row จาก rowIndex หรือเคสที่ inventory preview ไม่พอ
  - ถ้า table ยัง `hasMore=true` ต้องเรียก chunk ถัดไปจนจบก่อนเสนอ bulk write; ถ้า text ถูกตัดให้ถามผู้ใช้ก่อน replace ทั้งก้อน
...
- `get_instruction_data_snapshot` ต้องใช้เมื่อ inventory preview ไม่พอ โดยเฉพาะก่อนแก้หลายจุดหรือแก้ด้วย rowIndex เพื่อให้ AI อ่านข้อมูลจริงครบหรืออ่านทุก chunk ก่อนเสนอ batch
