...
- `get_instruction_data_snapshot`
  ...
  - AI ต้องเห็น `rowCount`/`totalRows` และเลือกอ่านช่วงได้ เช่น `rowNumberStart=50,rowNumberEnd=100` หรือ `startRow/endRow` แบบ rowIndex
  - ต้องใช้ก่อนงานที่แก้หลายจุด, replace ทั้ง text, ลบ/ย้าย/แก้ row จาก rowIndex หรือเคสที่ inventory preview ไม่พอ
  - ไม่มี cap แถวระดับ tool; ถ้า table ยัง `hasMore=true` ต้องเรียก chunk ถัดไปจนจบก่อนเสนอ bulk write
