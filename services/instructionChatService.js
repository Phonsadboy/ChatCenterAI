...
class InstructionChatService {
    ...
    getToolDefinitions() {
        return [
            ...
            { type: "function", function: { name: "update_followup_settings", description: "แก้ไขการตั้งค่าระบบติดตามลูกค้า — ระบุ pageKeys (array) เพื่อแก้เฉพาะเพจหลายเพจพร้อมกัน; ถ้าคำสั่งไม่ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด ให้เรียก list_followup_pages หรือถามหาเพจก่อน ห้ามปล่อย pageKeys ว่างจากคำสั่งกำกวม", parameters: { type: "object", properties: { autoFollowUpEnabled: { type: "boolean", description: "เปิด/ปิดระบบติดตามอัตโนมัติ" }, orderPromptInstructions: { type: "string", description: "คำสั่ง prompt สำหรับวิเคราะห์ออเดอร์" }, pageKeys: { type: "array", items: { type: "string" }, description: "รายการ pageKey เพื่อแก้เฉพาะเพจ — รองรับหลายเพจพร้อมกัน; เว้นว่างได้เฉพาะเมื่อผู้ใช้ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด" } } } } },
            { type: "function", function: { name: "update_followup_round", description: "แก้ไขข้อความหรือ delay ของ round ติดตามลูกค้า — ระบุ pageKeys เพื่อแก้หลายเพจพร้อมกัน; ถ้าคำสั่งไม่ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด ให้เรียก list_followup_pages หรือถามหาเพจก่อน ห้ามปล่อย pageKeys ว่างจากคำสั่งกำกวม", parameters: { type: "object", properties: { roundIndex: { type: "number", description: "ลำดับ round (0-indexed)" }, message: { type: "string", description: "ข้อความใหม่" }, delayMinutes: { type: "number", description: "ระยะเวลารอ (นาที)" }, pageKeys: { type: "array", items: { type: "string" }, description: "รายการ pageKey เพื่อแก้เฉพาะเพจ; เว้นว่างได้เฉพาะเมื่อผู้ใช้ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด" } }, required: ["roundIndex"] } } },
            { type: "function", function: { name: "manage_followup_images", description: "เพิ่มหรือลบรูปภาพใน round ติดตามลูกค้า — ระบุ pageKeys เพื่อแก้หลายเพจพร้อมกัน; ถ้าคำสั่งไม่ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด ให้เรียก list_followup_pages หรือถามหาเพจก่อน ห้ามปล่อย pageKeys ว่างจากคำสั่งกำกวม", parameters: { type: "object", properties: { roundIndex: { type: "number", description: "ลำดับ round (0-indexed)" }, action: { type: "string", enum: ["add", "remove"] }, assetId: { type: "string", description: "ID ของ asset จาก list_followup_assets" }, imageUrl: { type: "string", description: "URL ของรูปภาพ (ถ้าไม่มี assetId)" }, pageKeys: { type: "array", items: { type: "string" }, description: "รายการ pageKey เพื่อแก้เฉพาะเพจ; เว้นว่างได้เฉพาะเมื่อผู้ใช้ระบุชัดว่าแก้ global/ทุกเพจ/ทั้งหมด" } }, required: ["roundIndex", "action"] } } },
