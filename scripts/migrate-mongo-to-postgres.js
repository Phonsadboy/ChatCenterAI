...
async function loadCompletedCheckpointNames() {
  ...
  return new Set(
    result.rows
      .map((row) => String(row?.checkpoint_name || "").trim())
      .filter(Boolean),
  );
}
  ...
  try {
    const client = pgClient;
      const resumeFromCheckpoint =
        ...
        String(process.env.CCAI_MIGRATION_RESUME_FROM_CHECKPOINT || "true").toLowerCase() !== "false";
      const completedCheckpoints = resumeFromCheckpoint
        ? await loadCompletedCheckpointNames()
        : new Set();
      const activeStatusProgressEvery = Math.max(
        100,
        Number(process.env.CCAI_MIGRATION_PROGRESS_ACTIVE_EVERY || 1000) || 1000,
      );
      const chatHistoryProgressEvery = Math.max(
        100,
        Number(process.env.CCAI_MIGRATION_PROGRESS_CHAT_EVERY || 1000) || 1000,
      );

      const shouldSkipCheckpoint = (checkpointName) =>
        resumeFromCheckpoint && completedCheckpoints.has(checkpointName);
      const markCheckpoint = async (checkpointName, rowCount) => {
        await writeCheckpoint(checkpointName, rowCount);
        completedCheckpoints.add(checkpointName);
      };
      ...
      await markCheckpoint("settings", settings.length);
      ...
      for (const [platform, collectionName] of botCollections) {
        ...
        await markCheckpoint(collectionName, docs.length);
      }
      ...
      await markCheckpoint("instructions_v2", instructionDocs.length);
      ...
      await markCheckpoint("instruction_versions", versionDocs.length);
      ...
      await markCheckpoint("instruction_assets", assetDocs.length);
      ...
      await markCheckpoint("image_collections", imageCollections.length);
      ...
      await markCheckpoint("openai_api_keys", apiKeys.length);

      if (shouldSkipCheckpoint("orders")) {
        console.log("[Migration] checkpoint orders already completed, skip");
      } else {
        ...
        await markCheckpoint("orders", orders.length);
      }

      if (shouldSkipCheckpoint("follow_up_status")) {
        console.log("[Migration] checkpoint follow_up_status already completed, skip");
      } else {
        ...
        await markCheckpoint("follow_up_status", followUpStatuses.length);
      }

      if (shouldSkipCheckpoint("follow_up_page_settings")) {
        console.log("[Migration] checkpoint follow_up_page_settings already completed, skip");
      } else {
        ...
        await markCheckpoint("follow_up_page_settings", followUpPageSettings.length);
      }

      if (shouldSkipCheckpoint("follow_up_tasks")) {
        console.log("[Migration] checkpoint follow_up_tasks already completed, skip");
      } else {
        ...
        await markCheckpoint("follow_up_tasks", followUpTasks.length);
      }

      if (shouldSkipCheckpoint("notification_channels")) {
        console.log("[Migration] checkpoint notification_channels already completed, skip");
      } else {
        ...
        await markCheckpoint("notification_channels", notificationChannels.length);
      }

      if (shouldSkipCheckpoint("notification_logs")) {
        console.log("[Migration] checkpoint notification_logs already completed, skip");
      } else {
        ...
        await markCheckpoint("notification_logs", notificationLogs.length);
      }

      if (shouldSkipCheckpoint("active_user_status")) {
        console.log("[Migration] checkpoint active_user_status already completed, skip");
      } else {
        ...
        for await (const doc of activeUserStatusCursor) {
          ...
          if (migratedActiveUserStatuses % activeStatusProgressEvery === 0) {
            console.log(`[Migration] active_user_status processed: ${migratedActiveUserStatuses}`);
          }
        }
        await markCheckpoint("active_user_status", migratedActiveUserStatuses);
      }

      if (shouldSkipCheckpoint("user_tags")) {
        console.log("[Migration] checkpoint user_tags already completed, skip");
      } else {
        ...
        await markCheckpoint("user_tags", userTags.length);
      }

      if (shouldSkipCheckpoint("user_purchase_status")) {
        console.log("[Migration] checkpoint user_purchase_status already completed, skip");
      } else {
        ...
        await markCheckpoint("user_purchase_status", purchaseStatuses.length);
      }

      if (shouldSkipCheckpoint("chat_feedback")) {
        console.log("[Migration] checkpoint chat_feedback already completed, skip");
      } else {
        ...
        await markCheckpoint("chat_feedback", feedbackDocs.length);
      }

      const profileMap = new Map();
      const contactIdCache = new Map();
      const threadIdCache = new Map();
      if (shouldSkipCheckpoint("user_profiles")) {
        console.log("[Migration] checkpoint user_profiles already completed, skip");
      } else {
        ...
        await markCheckpoint("user_profiles", migratedProfiles);
      }

      if (shouldSkipCheckpoint("chat_history")) {
        console.log("[Migration] checkpoint chat_history already completed, skip");
      } else {
        ...
        for await (const doc of messagesCursor) {
          ...
          migratedMessages += 1;
          if (migratedMessages % chatHistoryProgressEvery === 0) {
            console.log(`[Migration] chat_history processed: ${migratedMessages}`);
          }
        }
        await markCheckpoint("chat_history", migratedMessages);
      }

      if (shouldSkipCheckpoint("short_links")) {
        console.log("[Migration] checkpoint short_links already completed, skip");
      } else {
        ...
        await markCheckpoint("short_links", shortLinks.length);
      }
  } finally {
